...
class Calendar(GAuth):
    def __init__(self,path):
        ...
        if not os.path.exists(self.credentials):
            ...
            self.service=self.auth(scopes=["https://www.googleapis.com/auth/calendar.readonly","https://www.googleapis.com/auth/tasks.readonly"],service_name="calendar",version="v3")
    def get_events(self,date=datetime.datetime.now()):
        ...
        all_events=sorted(all_events,key=lambda x:datetime.datetime.fromisoformat(x["start"].get("dateTime", x["start"].get("date"))).replace(tzinfo=datetime.timezone.utc))
        return all_events
    ...
    def set_today_events(self):
        ...
        for event in today_events:
            ...
            if event_date.replace(tzinfo=datetime.timezone(datetime.timedelta(hours=9))) <= datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=9))):
                continue
            self.add_notification(f"{event_title}の予定が1時間後にあります",timestamp=event_date.timestamp()-3600) #1時間前
            self.add_notification(f"{event_title}の予定が30分後にあります",timestamp=event_date.timestamp()-1800) #30分前
...
class Tasks(GAuth):
    def __init__(self,path):
        super().__init__(path)
        self.service=self.auth(scopes=["https://www.googleapis.com/auth/tasks.readonly","https://www.googleapis.com/auth/calendar.readonly"],service_name="tasks",version="v1")
    def get_todos(self,date=datetime.datetime.now()):
        ...
        tasks=sorted(tasks,key=lambda x:datetime.datetime.fromisoformat(x["due"]))
        return tasks
