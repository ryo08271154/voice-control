...
class RandomPlugin(BasePlugin):
    ...
    def execute(self, command):
        ...
        text = text.replace("一", "1").replace("二", "2").replace("三", "3").replace("四", "4").replace("五", "5").replace(
            "六", "6").replace("七", "7").replace("八", "8").replace("九", "9").replace("十", "10").replace("百", "100")
        ...
        command.flet_view = ft.Container(ft.Text(
            random_number, size=100, text_align=ft.TextAlign.CENTER), alignment=ft.alignment.center, expand=True)
...
class DicePlugin(BasePlugin):
    ...
    def execute(self, command):
        ...
        command.flet_view = ft.Container(ft.Text(
            DICE_EMOJI[dice_number - 1], size=100, text_align=ft.TextAlign.CENTER), alignment=ft.alignment.center, expand=True)
        return super().execute(command)
    ...
    def execute(self, command):
        ...
        for url in rss_urls:
            ...
            for item in items:
                ...
                topic = ft.Container(content=ft.Column([ft.Text(title, size=20), ft.Text(description), ft.Text(
                    rss_title.text)]), bgcolor=ft.Colors.WHITE10, padding=10, border_radius=5)
...
class SearchPlugin(BasePlugin):
    ...
    def execute(self, command):
        text = command.user_input_text
        query = text.replace("検索", "").replace("ウェブ", "").replace("する", "").replace("して", "").replace("で", "").replace("を", "").replace("グーグル", "").replace("Google", "").replace("youtube", "").replace("ユーチューブ", "").replace("ティックトック", "").replace(
            "ツイッター", "").replace("ウィキペディア", "").replace("アマゾン", "").replace("スポティファイ", "").replace("マップ", "").replace("地図", "").replace("YouTube", "").replace("TikTok", "").replace("Twitter", "").replace("Wikipedia", "").replace("Amazon", "").replace("Spotify", "")
        search_url = None
        service_name = None
    ...
    def execute(self, command):
        ...
        if total_seconds > 0:
            ...
            self.add_notification(
                f"{minutes}分{seconds}秒のタイマーが終了しました。", timestamp=time.time()+total_seconds)
        else:
            if self.is_plugin_mode == False:
