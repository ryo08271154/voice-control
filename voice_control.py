...
words.extend(devices_name)
words.extend(custom_devices_name)
def always_on_voice():
    global last_text
    ...
    if r['intents']:
        if r['entities']:
            if r['intents'][0]['name'] in ['turnOn','turnOff']:
                ...
                custom_device_control(device_name,action)
                switchbot_device_control(device_name,action)
...
def control(text):
    ...
    if reply!="":
        reply="よくわかりませんでした"
def custom_device_control(text,action):
...
def custom_scene_control(text):
    for i in custom_scenes["sceneList"]:
        if i["sceneName"] in text:
            ...
            subprocess.run(command)
            yomiage(reply)
def switchbot_device_control(text,action):
    for i in text:
        if i in devices_name:
            print(action)
            if action:
